...
let tempDirs: string[] = []
const TEST_FILE_PATH = "/tmp/vispark-code-test-keybindings.json"
...
describe("KeybindingsManager", () => {
  ...
  test("uses the runtime profile for the default keybindings path", () => {
    const previous = process.env.VISPARK_RUNTIME_PROFILE
    process.env.VISPARK_RUNTIME_PROFILE = "dev"

    const manager = new KeybindingsManager()

    expect(manager.filePath).toEndWith("/.vispark-code-dev/keybindings.json")

    if (previous === undefined) {
      delete process.env.VISPARK_RUNTIME_PROFILE
    } else {
      process.env.VISPARK_RUNTIME_PROFILE = previous
    }
  })
})
