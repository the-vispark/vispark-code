...
const SKILL_AGENT_ALIASES = ["universal"] as const
...
export function getGlobalSkillsRoot(lockFilePath = getGlobalSkillLockPath()) {
  const lockDirectory = path.dirname(lockFilePath)
  return path.basename(lockDirectory) === "skills"
    ? lockDirectory
    : path.join(lockDirectory, "skills")
}

export function getVisionSkillsRoot() {
  const configDir = process.env.Vision_CONFIG_DIR?.trim() || path.join(os.homedir(), ".Vision")
...
export async function linkSkillIntoVision(skillId: string, lockFilePath = getGlobalSkillLockPath()) {
  const normalizedSkillId = assertSafeSkillId(skillId)
  const sourcePath = path.join(getGlobalSkillsRoot(lockFilePath), normalizedSkillId)
  const sourceSkillPath = path.join(sourcePath, "SKILL.md")
  if (!await pathExists(sourceSkillPath)) {
    throw new Error(`Installed skill was not found at ${sourceSkillPath}.`)
  }

  const visionSkillsRoot = getVisionSkillsRoot()
  const targetPath = path.join(visionSkillsRoot, normalizedSkillId)
  await mkdir(visionSkillsRoot, { recursive: true })
  ...
  try {
    ...
    if (targetStats.isSymbolicLink()) {
      ...
      throw new Error(`Vision skill target already exists at ${targetPath}.`)
    }
  } catch (error) {
...
export async function unlinkSkillFromVision(skillId: string) {
  const normalizedSkillId = assertSafeSkillId(skillId)
  const targetPath = path.join(getVisionSkillsRoot(), normalizedSkillId)
  try {
    const targetStats = await lstat(targetPath)
    if (targetStats.isSymbolicLink()) {
      await rm(targetPath)
...
export async function installSkill(source: string, skillId: string): Promise<SkillInstallResult> {
  ...
  await linkSkillIntoVision(command[5])
  return {
    source: command[3],
    skillId: command[5],
    command,
...
export async function uninstallSkill(skillId: string): Promise<SkillUninstallResult> {
  ...
  const { cwd, stdout, stderr } = await runSkillCommand(command)
  await unlinkSkillFromVision(command[3])
  return {
    skillId: command[3],
    command,
    cwd,
