...
const originalRuntimeProfile = process.env.VISPARK_RUNTIME_PROFILE
const tempDirs: string[] = []

afterEach(async () => {
  if (originalRuntimeProfile === undefined) {
    delete process.env.VISPARK_RUNTIME_PROFILE
  } else {
    process.env.VISPARK_RUNTIME_PROFILE = originalRuntimeProfile
  }
...
async function createTempDataDir() {
  const dir = await mkdtemp(join(tmpdir(), "vispark-event-store-"))
  tempDirs.push(dir)
  return dir
}
...
describe("EventStore", () => {
  test("uses the runtime profile for the default data dir", () => {
    ...
    expect(store.dataDir).toEndWith("/.vispark-code-dev/data")
  })
