...
export class VisparkCodeSocket {
  ...
  subscribeFileTree(
    projectId: string,
    handlers: {
      onSnapshot: SnapshotListener<{ projectId: string; rootPath: string; pageSize: number; supportsRealtime: true } | null>
  ...
  private connect() {
    ...
    this.ws.addEventListener("message", (event) => {
      ...
      if (isSendToStartingProfilingEnabled() && payload.type === "snapshot" && payload.snapshot.type === "chat" && payload.snapshot.data?.runtime.status === "starting") {
        console.debug("[vispark-code/send->starting][client-ws]", {
