...
import { buildResolvedTranscriptRows, VisparkCodeTranscriptRow, type ResolvedTranscriptRow } from "./VisparkCodeTranscript"
...
export function shouldShowTranscriptTocPanel(args: {
  ...
  return args.enabled && args.layoutWidth > 1200 && args.itemCount > 0
}
...
interface ChatTranscriptViewportProps {
  scrollRef: VisparkCodeState["scrollRef"]
  messages: VisparkCodeState["messages"]
...
const ChatTranscriptViewport = memo(function ChatTranscriptViewport({
  ...
  const rowVirtualizer = useVirtualizer({
    ...
    getItemKey: (index: number) => `${virtualMeasurementScopeKey}:${virtualizedHeadRows[index]?.id ?? index}`,
    estimateSize: (index: number) => estimateTranscriptRowHeight(virtualizedHeadRows[index] ?? pinnedTailRows[0]!),
    measureElement,
  ...
  useEffect(() => {
    rowVirtualizer.shouldAdjustScrollPositionOnItemSizeChange = (item: any, _delta: number, instance: any) => {
        ...
        >
          <div className="mx-auto flex h-full max-w-[800px] items-center justify-center">
            <div className="flex flex-col items-center justify-center text-muted-foreground gap-4 opacity-70">
              <BrandMark className="size-16 rounded-2xl vispark-code-empty-state-flower" />
              <div
                className="text-base font-normal text-muted-foreground text-center max-w-xs flex items-center vispark-code-empty-state-text"
              ...
              >
                <span className="relative inline-grid place-items-start">
                  <span className="invisible col-start-1 row-start-1 whitespace-pre flex items-center">
                    ...
                    <span className="vispark-code-typewriter-cursor-slot" aria-hidden="true" />
                  </span>
                  <span className="col-start-1 row-start-1 whitespace-pre flex items-center">
                    <span>{typedEmptyStateText}</span>
                    <span className="vispark-code-typewriter-cursor-slot" aria-hidden="true">
                      <span
                        className="vispark-code-typewriter-cursor"
...
interface ChatInputDockProps {
  ...
  availableProviders: VisparkCodeState["availableProviders"]
  contextWindowSnapshot: ContextWindowSnapshot | null
  onSubmit: VisparkCodeState["handleSend"]
  onCancel: () => void
}
    ...
    <div className="absolute bottom-0 left-0 right-0 z-20 pointer-events-none">
      <div className="bg-gradient-to-t from-background via-background pointer-events-auto" ref={inputRef}>
        <ChatInput
          ...
          missingVisionApiKey={settingsReady && !hasVisionApiKey}
...
interface TerminalWorkspaceShellProps {
  ...
  socket: VisparkCodeState["socket"]
  connectionStatus: VisparkCodeState["connectionStatus"]
  scrollback: number
    ...
    >
      <CardContent className="flex flex-1 min-h-0 flex-col p-0 overflow-hidden relative">
        ...
        {shouldShowTranscriptToc ? (
          ...
          >
            <div className="rounded-2xl border border-border bg-background/90 px-1 py-1 backdrop-blur-md" data-testid="transcript-toc">
      ...
      <ChatInputDock
        ...
        hasVisionApiKey={state.hasVisionApiKey}
        runtimeStatus={state.runtime?.status ?? null}
        canCancel={state.canCancel}
        projectId={projectId}
